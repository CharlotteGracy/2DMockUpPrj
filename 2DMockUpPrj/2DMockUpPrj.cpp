...
// 2DMockUpPrj.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
...
// _In_: SAL 주석 - 자주 사용되는 주석을 키워드로 작성함.
// hInstance : 실행된 프로세스의 시작 주소, 인스턴스 
// hPrevInstance :이전에 실행된 인스턴스 핸들 
// IpCmdLine : 명령으로 입력된 프로그램의 인수
                     ...
                     _In_ int       nCmdShow)
{
    ...
    // 메시지 큐에 msg.message == WM_QUIT 인 경우 False 반환
    MSG msg;
    ...
    while (GetMessage(&msg, nullptr, 0, 0))
    {
        if(PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
            ...
            {
              TranslateMessage(&msg); // 키보드 입력메시지 처리
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   SetWindowPos(hWnd, NULL, WINSTARTX, WINSTARTY, (rc.right - rc.left),(rc.top-rc.bottom), SWP_NOZORDER | SWP_NOMOVE);
...
// WndProc: 메시지를 운영체제에 전달한다. 강제로 운영체제 호출.
// hWnd: 메시지가 어느 윈도우를 대상으로 전달된 것인지 구분한다.
// message: 메시지 구분 번호
// wParam: unsigned int 메시지의 매개변수 1
// lParam: unsinged long 메시지의 매개변수 2
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        break;
    case WM_PAINT: // 윈도우의 작업영역이 다시 그려져야 할 때 실행됨. (무효화 영역이 발생했을 때.)
        {
            PAINTSTRUCT ps;
